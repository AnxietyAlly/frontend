<script>
			...
			.then((questionLinksJSON) => {
				...
				Promise.all(questionPromises)
					.then((questions) => {
						...
						//		Display questions in the 'questionposition' element
						const questionPositionElement = document.getElementById('questionposition');
						// questionsFromDatabase.forEach((question) => {
						// 	const li = document.createElement('li');
						// 	console.log(question.data.question);
						// 	li.textContent = question.data.question;
						// 	questionPositionElement.appendChild(li);
						// });
						const li2 = document.createElement('li');
						li2.textContent = questionsFromDatabase[3].data.question;
						questionPositionElement.appendChild(li2);
					})
	...
	let questions = [
		{
			question:
				'Question 1: How often do you feel a sense of excessive worry or fear without a specific cause.',
			options: ['Frequently', 'Never', 'Not Often', 'Sometimes'],
			correctIndex: 3
		},
		{
			question:
				"Question 2: Do you often feel tense or anxious? For example, during a specific situation or when you're about to go to sleep.",
			options: ['Frequently', 'Never', 'Not often', 'Sometimes'],
			correctIndex: 3
		},
		{
		...
		{
			question:
				'Question 4: Do you worry? For example, about what others think of you in certain situations?',
			options: ['Frequently', 'Never', 'Not often', 'Sometimes'],
			correctIndex: 1
		},
		{
			question:
				'Question 5: Do you avoid social gatherings because of fear, anxiety, or nervousness?',
			options: ['Frequently', 'Never', 'Not often', 'Sometimes'],
			correctIndex: 2
		},
		{
		...
		{
			question:
				'Question 8: Do you experience physical symptoms such as shortness of breath, sweating, restlessness, tingling, or nausea?',
			options: ['Frequently', 'Never', 'Not often', 'Sometimes'],
			correctIndex: 1
		},
		{
			question:
				"Question 9: Do you have episodes of anxiety and fear that you can't seem to control?",
			options: ['Frequently', 'Never', 'Not often', 'Sometimes'],
			correctIndex: 0
		}
	];
...
<div class="min-h-screen bg-sky flex flex-col items-center">
	...
	<div class="app top-0">
		{#if questionPointer == -1}
			...
			<ul id="questionposition" />
			<ul id="singleQuestionTest" />
			<button
